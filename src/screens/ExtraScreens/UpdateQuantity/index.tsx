...
import {updaCart, updateProducts, allProducts} from '../../../redux/actions';
...
const UpdateQuantity = ({navigation}) => {
  ...
  const {UCart, updatedProducts, forDetails, allProds, totalProducts} =
    useSelector(({BCART}) => BCART);
  ...
  const filter = e => {
    const filterArrs = products.filter(prodItem =>
      prodItem.products.find(i => i.name.includes(`${e.toUpperCase()}`)),
    );
